...
let treatmentTypeRouter = express.Router();
treatmentTypeRouter.use(verifyToken);

treatmentTypeRouter.post('/', async (req, res, next) => {
    try{
        let treatmentType = await treatmentTypeService.createTreatmentType(req.body);
...
treatmentTypeRouter.get('/', async (req, res, next) => {
    try{
        let treatmentTypes = await treatmentTypeService.findTreatmentTypes();
...
treatmentTypeRouter.get('/:treatmentTypeId', async (req, res, next) => {
    try{
        let treatmentType = await treatmentTypeService.findTreatmentType(req.params.treatmentTypeId);
...
treatmentTypeRouter.put('/:treatmentTypeId', async (req, res, next) => {
    try{
        let [_, treatmentTypes] = await treatmentTypeService.updateTreatmentType(req.params.treatmentTypeId, req.body);
        return res.status(200).json(JSON.stringify(treatmentTypes));
    }
    catch(e){
        next(e);
