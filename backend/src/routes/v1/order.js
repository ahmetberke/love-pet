...
orderRouter.get('/', async (req, res, next) => {
    try{
        let orders = await orderService.findOrders();
...
orderRouter.get('/:orderId', async (req, res, next) => {
    try{
        let order = await orderService.findOrder(req.params.orderId);
        if(order !== null){
            return res.status(200).json(order.toJSON());
        }
        else{
...
orderRouter.put('/:orderId', async (req, res, next) => {
    try{
        let [_, orders] = await orderService.updateOrder(req.params.orderId, req.body);
        return res.status(200).json(JSON.stringify(orders));
    }
    catch(e){
        next(e);
