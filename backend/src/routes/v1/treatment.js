...
let treatmentRouter = express.Router();
treatmentRouter.use(verifyToken);

treatmentRouter.post('/', async (req, res, next) => {
    try{
        let treatment = await treatmentService.createTreatment(req.body);
...
treatmentRouter.get('/', async (req, res, next) => {
    try{
        let treatments = await treatmentService.findTreatments();
...
treatmentRouter.get('/:treatmentId', async (req, res, next) => {
    try{
        let treatment = await treatmentService.findTreatment(req.params.treatmentId);
        if(treatment !== null){
            return res.status(200).json(treatment.toJSON());
        }
        else{
...
treatmentRouter.put('/:treatmentId', async (req, res, next) => {
    try{
        let [_, treatments] = await treatmentService.updateTreatment(req.params.treatmentId, req.body);
        return res.status(200).json(JSON.stringify(treatments));
    }
    catch(e){
        next(e);
