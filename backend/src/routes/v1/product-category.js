...
productCategoryRouter.post('/', async (req, res, next) => {
    try{
        let productCategory = await productCategoryService.createProductCategory(req.body);
...
productCategoryRouter.get('/', async (req, res, next) => {
    try{
        let productCategories = await productCategoryService.findProductCategories();
...
productCategoryRouter.get('/:productCategoryId', async (req, res, next) => {
    try{
        let productCategory = await productCategoryService.findProductCategory(req.params.productCategoryId);
...
productCategoryRouter.put('/:productCategoryId', async (req, res, next) => {
    try{
        let [_, productCategories] = await productCategoryService.updateProductCategory(req.params.productCategoryId, req.body);
        return res.status(200).json(JSON.stringify(productCategories));
    }
    catch(e){
        next(e);
