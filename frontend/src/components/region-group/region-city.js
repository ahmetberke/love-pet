...
function RegionCity(props) {
  ...
  const onCityChange = (event) => {
    ...
    setCityValidationMsg(
        (selectedCityId === -1) ? 'You should select city' : '');
  ...
  const slctedProvinces = (selectedCityId === -1 || selectedCity.length === 0) ?
    [] : selectedCity[0].Provinces;

  return (
    <>
      <Col>
        <Form.Group className="mb-3">
          <OverlayTriggerWrapper msg={cityValidationMsg}>
      ...
      <RegionProvince provinces={slctedProvinces}
